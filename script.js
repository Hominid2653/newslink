...
function showPage(pageId) {
  ...
  if (pageId === "newsPage") {
    ...
    if (!container || container.children.length === 0) loadNews(true);
    renderPreferences();
  }
...
function login() {
  ...
  if (!users[username]) return alert("Invalid login");

  const stored = users[username].password;
  const encoded = encodePassword(password);
  const plainTextMatch = stored === password;
  const encodedMatch = stored === encoded;

  if (!plainTextMatch && !encodedMatch) return alert("Invalid login");

  if (plainTextMatch) {
    users[username].password = encoded;
    saveUsers(users);
  }
...
function renderCategoryPicker() {
  ...
  CATEGORIES.forEach(cat => {
    ...
    btn.addEventListener("click", () => {
      ...
      document.getElementById("prefCategory").value = selectedCategory;

      container.querySelectorAll(".category-tag").forEach(b => {
        const isSelected = b.dataset.value === selectedCategory;
        b.className = "category-tag px-3 py-1.5 rounded-full text-sm font-medium border transition-all " + (
          isSelected
...
const PLACEHOLDER_IMG = "https://placehold.co/400x200?text=No+Image";
...
async function loadNews(reset = false, query = "") {
  ...
  try {
    ...
    const searchQuery = currentSearchQuery || allKeywords.join(" ") || "";
...
function renderSavedArticles() {
  ...
  if (!articles.length) {
    container.innerHTML = `
      <p class="col-span-full text-center text-gray-500 py-10">No saved articles yet ⭐</p>
    `;
    return;
  }
  ...
  articles.forEach(article => {
    ...
    card.innerHTML = `
      <div class="absolute top-2 left-2 bg-green-100 text-green-700 px-2 py-1 text-xs rounded">Saved</div>
      <img src="${article.urlToImage || PLACEHOLDER_IMG}"
           onerror="this.src='${PLACEHOLDER_IMG}'"
           class="w-full h-48 object-cover">
      <div class="p-4 flex flex-col flex-grow">
        ...
        <div class="mt-auto flex flex-col gap-2">
          <div class="flex justify-between items-center">
            <span class="text-xs text-gray-500">${article.source?.name || "Unknown"}</span>
            <button class="remove-btn text-xl text-yellow-500">⭐</button>
          </div>
          <a href="${article.url}" target="_blank"
             class="bg-blue-600 text-white text-center py-2 rounded hover:bg-blue-700">
             Read Article
